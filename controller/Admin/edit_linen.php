...
if (!empty($_POST['id_linen'])) {
    ...
    //untuk mengecek ada linen yang sama tidak
    ...
    if ($nama_linen !== $data_linen['nama_linen'] && $rows_cek_linen > 0) {
        ...
        if ($update_linen->execute()) {
            $update_linen->close();
            header('location:' . $base_url . 'admin/linen/list/?message_success=Data Linen Berhasil Diedit!.');
        } else {
            header('location:' . $base_url . 'admin/linen/list/?message_failed=Data Linen Gagal Diedit!.');
        }
    }
}else{
	header('location:' . $base_url . 'admin/linen/list/?message_failed');
