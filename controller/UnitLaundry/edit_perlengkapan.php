...
if (!empty($_POST['id_perlengkapan'])) {
    ...
    if ($nama_perlengkapan !== $data_perlengkapan['nama_perlengkapan'] && $rows_cek_perlengkapan > 0) {
        ...
        if ($update_perlengkapan->execute()) {
            ...
            header('location:' . $base_url . 'laundry/perlengkapan/?message_success=Selamat, Data Perlengkapan Berhasil Diedit');
        } else {
            header('location:' . $base_url . 'laundry/perlengkapan/?message_failed=Maaf, Data Perlengkapan gagal Diedit!.');
        }
    }
}else{
	header('location:' . $base_url . 'admin/perlengkapan/?message_failed');
