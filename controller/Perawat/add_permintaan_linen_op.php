...
$keterangan = trim(strtolower(filterString($_POST['keterangan'])));
	...
	if ($insertLinenPengajuan) {
		header('location:'.$base_url.'perawat/linen/daftar-linen/?message_success=Selamat, Permintaan Linen Berhasil ditambahkan!!!');
	}else{
		header('location:'.$base_url.'perawat/linen/daftar-linen/?message_failed=Maaf, Permintaan Linen Gagal ditambahkan'.mysqli_error($conn));
	}
