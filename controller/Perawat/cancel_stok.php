...
//get sisa
$sqlSisa = mysqli_query($conn, "SELECT penerimaan_linen_baru.jml_diterima, permintaan_linen_baru.jml_permintaan FROM penerimaan_linen_baru INNER JOIN permintaan_linen_baru ON permintaan_linen_baru.id_permintaan_linen_baru=penerimaan_linen_baru.id_permintaan_linen_baru WHERE penerimaan_linen_baru.id_penerimaan_linen_baru = $id_penerimaan");
...
if ($rows > 0) {
	...
	if ($sqlUpdate) {
		$sqlUpdateLinen = mysqli_query($conn, "UPDATE `linen` SET `jml_linen`= $sisaTotal WHERE `id_penerimaan_linen_baru` = $id_penerimaan");
		if ($sqlUpdateLinen) {
